...
export default function OutreachDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (statsResponse.ok) {
        const statsData: OutreachStats = await statsResponse.json();
        setStats(statsData);
      }

      // Fetch outreach records
      const outreachResponse = await fetch(
        `${API_URL}/api/v1/admin/outreach?limit=500`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
      ...
      if (outreachResponse.ok) {
        const outreachData: OutreachRecord[] = await outreachResponse.json();
  ...
  useEffect(() => {
    ...
    if (showFollowUpsOnly) {
      ...
      filtered = filtered.filter(
        (r) => r.next_follow_up_date && new Date(r.next_follow_up_date) <= today
      );
    }
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {showFollowUpsOnly
                  ? 'Total Outreach'
                  : `Due Today (${stats?.pending_followups || 0})`}
              </Button>
            </div>
          </div>
        </div>
              ...
              <table className="w-full">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredOutreach.map((record) => (
                    <tr
                      key={record.id}
                      className={`hover:bg-gray-50 cursor-pointer ${getPriorityColor(
                        record.priority
                      )}`}
                      onClick={() => {
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-sm text-gray-900">
                        <span className="font-medium">
                          {record.contact_attempt_count || 0}
                        </span>
                      </td>
                      <td className="px-4 py-4">
...
function OutreachDetailModal({
  ...
  const handleSubmitFollowUp = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/v1/admin/outreach/${record.id}`,
        {
          method: 'PUT',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const timelineEntries = (record.notes || '')
    .split('\n')
